...
class ReleaseOperationsTests(unittest.TestCase):
    ...
    def test_deployment_and_smoke_instructions_match_the_release_artifacts(self) -> None:
        ...
        self.assertEqual(candidate_id, "0.1.0-player-cards-preview-p1-r2-20260909-local")
        stale_id = "0.1.0-reporting-heatmaps-excel-p1-20260827-local"
        active_baseline = "0.1.0-player-cards-preview-p1-r2-20260909-local"
        ...
        manifest_digest = re.search(
            r"^\| (?:Candidate )?[Rr]untime digest ×2 \| `(sha256:[0-9a-f]{64})` \|$",
            manifest,
            re.MULTILINE,
        )
